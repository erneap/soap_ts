...
router.get('/plans', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (plansCol) {
        const cursor = yield plansCol.find({});
        const plans = yield cursor.toArray();
        const list = [];
        plans.forEach(p => {
            list.push(new plans_1.Plan(p));
        });
...
router.get('/plan/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const plansCol = mongoconnect_1.collections.plans;
    if (plansCol) {
        const id = new mongodb_1.ObjectId(req.params.id);
        const query = { _id: id };
        const result = yield plansCol.findOne(query);
        if (result && result !== null) {
            const plan = new plans_1.Plan(result);
...
router.post('/plan/newplan', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const plansCol = mongoconnect_1.collections.plans;
    const data = req.body;
    if (plansCol) {
...
router.post('/plan/reading', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const plansCol = mongoconnect_1.collections.plans;
    const data = req.body;
    if (plansCol) {
        ...
        if (result && result !== null) {
                ...
                if (month.month === data.month) {
                    ...
                    for (let d = 0; d < month.days.length && !dFound; d++) {
                        ...
                        if (day.dayOfMonth === data.day) {
                            ...
                            const reading = new plans_1.Reading();
            ...
            yield plansCol.replaceOne(query, plan);
            return res.status(200).json(plan);
        }
        else {
                                        ...
                                        switch (data.field.toLowerCase()) {
                                            ...
                                            case "setreadings":
                                                ...
                                                if (iRead < day.readings.length) {
                                                    ...
                                                    while (day.readings.length < iRead) {
                                                        day.readings.push(new plans_1.Reading());
                                                    }
                                                }
                                                for (let r = 0; r < day.readings.length; r++) {
                                                    day.readings[r].id = r + 1;
                                            ...
                                            case "addreading":
                                                ...
                                                const newRead = new plans_1.Reading({
                                                    id: next + 1,
                                                    book: '',
                                                    chapter: 0
                                                });
                            ...
                            switch (data.field.toLowerCase()) {
                                ...
                                case "days":
                                    ...
                                    if (idays < month.days.length) {
                                        ...
                                        while (month.days.length < idays) {
                                            month.days.push(new plans_1.PlanDay());
                                        }
                                    }
                                ...
                                case "add":
                                    ...
                                    for (let i = 0; i < days; i++) {
                                        const newday = new plans_1.PlanDay({
                                            dayOfMonth: max + i + 1,
                                            readings: []
                                        });
                    ...
                    case "months":
                        ...
                        if (plan.months.length > iMonths) {
                            ...
                            for (let i = plan.months.length; i < iMonths; i++) {
                                const month = new plans_1.PlanMonth();
                                month.month = i;
...
router.delete('/plan/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const plansCol = mongoconnect_1.collections.plans;
    if (plansCol) {
        const id = new mongodb_1.ObjectId(req.params.id);
        const query = { _id: id };
        const result = yield plansCol.deleteOne(query);
